...
const { data: serverCatalogInfo, pending: pending, error: error } = await useAsyncData(
  () => `getMallProductCatalogById-${cateid}`, () => {
    return getMallProductCatalogById({ id: cateid })
  }
)
const initData = (dataInfo) => {
  ...
  if (result?.id) {
    ...
    const links: any = [{ label: 'Home', to: convertToAbsolutePath('/') }]

    if (bannerInfo.value.pathIdList?.length > 0) {
      bannerInfo.value.pathIdList.forEach((id: string, index: number) => {
        links.push({
          label: bannerInfo.value.pathNameList[index],
          to: convertToAbsolutePath(`/categories/${slugify(bannerInfo.value.pathNameList[index])}-${id}`),
          disabled: index === bannerInfo.value.pathIdList.length - 1
        })
      })
    }
    ...
    useServerSeoMeta({
      ...
      throwPageError('Category not found or not published.')
  }
}
initData(serverCatalogInfo.value?.result || {})
...
const createJsonLdBreadcrumb = (links) => {
  ...
  createJsonLd(breadcrumbParams)
}
watchEffect(() => {
  console.log('面包屑变化---:', breadcrumbLinks.value)
  if (isPageLoaded.value && breadcrumbLinks.value.length > 0) {
    createJsonLdBreadcrumb(breadcrumbLinks.value);
  }
});
    ...
    <div class="max-row py-4">
      ...
      <div class="relative h-[330px] overflow-hidden max-md:w-full max-md:h-auto max-md:aspect-[2/1]"
        ...
        <div class="absolute inset-0  md:block hidden">
          <NuxtImg :src="categorybanner" class="w-full h-full object-cover object-center rounded-lg"
            :alt="categoryalt" />
        </div>
