            ...
            <div class="sticky top-[100px] z-[49]">
              <ClientOnly>
                ...
                <div class="relative"
                  ...
                  <Swiper :modules="[Navigation, Controller]" ref="swiperRefMain" :space-between="10"
                    ...
                    <SwiperSlide v-if="currentSkuImage" :key="'sku-' + currentSkuImage.url"
                      ...
                      <template v-else>
                        <NuxtImg fetchpriority="high" loading="eager" format="webp"
                          :src="`${item.url}?x-oss-process=image/auto-orient,1/resize,w_800,m_lfit` || '/home/place_img.png'"
                          placeholder="/home/place_img.png"
                ...
                <div v-else
                  ...
                  <NuxtImg format="webp" src="/images/empty.jpg" loading="loading"
                ...
                <Swiper v-if="productinfo.photoList && productinfo.photoList.length > 0"
                  ...
                  <SwiperSlide v-for="(item, idx) in productinfo.photoList" :key="'thumb-' + (item.url || idx)"
                    ...
                    :class="{ 'border-primary border-[1px]  max-w-[50px] md:max-w-[90px] rounded-[4px]': isThumbActive(idx) }">
                    <div
                      class="relative w-full  max-w-[50px] md:max-w-[90px] aspect-square rounded-[4px] p-2 max-md:p-1 overflow-hidden cursor-pointer hover:opacity-80 bg-[#F8F8F8]">
                      <template v-if="isVideo(item.url)">
          ...
          <NuxtLink :to="`/products/${slugify(product.seoUrlKeyword || product.productEnglishName)}-${product.id}`"
            ...
            <div class="mt-2">
              ...
              <p class="text-sm font-medium text-primary">
                ...
                <!-- Crossed-out price -->
                <span v-if="product.originPrice" class="text-[16px] md:text-[18px] text-gray-400 line-through ml-3">
                  ${{ product.originPrice }}
                </span>
              </p>
            </div>
...
const serviceList = [
  ...
  { img: '/product/service_2.webp', text: 'Energy Efficient' },
...
if (lastpage) {
  ...
  // 1. 匹配普通分类 /abc-123
  const categoryMatch = decodedPath.match(/^\/products\/([^/?#]+)/);
  if (categoryMatch) {
    const fullSlug = categoryMatch[1];
    ...
    lastLabel = categoryName;
    lastTo = lastpage;
    // console.log('====categoryMatch=========',categoryMatch,lastLabel,lastTo)
  }
