<template>
  <div class="bg-white w-full pt-4 font-hind">
    ...
    <div v-else class="max-row">
      <CustomBreadcrumb :links="breadcrumbLinks" wrapper-class="mb-4"/>
      ...
      <div class="text-gray-800" ref="detailSectionRef">
        <div class="flex flex-row max-md:flex-col gap-6 max-md:gap-4 border-b border-b-[#D1D1D1] pb-4">
          ...
          <div class="w-[57%] h-full max-md:w-full">
            ...
            <!-- Section 服务类 -->
            <div class="flex items-center flex-wrap gap-2 text-[#0C1013] text-[14px] font-normal">
            ...
            <div v-if="productinfo.normalPropertyList" class="mt-2">
              <div class="py-2" :data-prop-block="index" v-for="(property, index) in productinfo.normalPropertyList"
                ...
                <div class="flex justify-between items-center cursor-pointer">
                  <h2 class="font-normal text-[14px] flex items-center mb-0 leading-normal flex-wrap gap-y-[2px]">
                ...
                <p v-if="property.proposedView && property.proposedViewType === 20 && property.proposedDesc"
                  @click.stop="openProposedDrawer(property)" class="ml-2 mt-2 text-[14px] text-[#5A5B5B] ">
                  How to Measure Shades or Blinds?<span
                    class="ml-2 text-primary hover:text-primary-600 underline cursor-pointer"> View Our Guide</span>
                </p>
                                    ...
                                    <template v-if="getInputConfig(needinput, pindex).viewType === 10">
                                      ...
                                      <div>
                                        ...
                                        <InputNumber type="number" v-model:value="needinput.inputvalue[pindex]"
                                          @change="changeinputvalue(property, needindex + 2, index)"
                                          class="custom-input rounded text-sm w-[250px] max-md:w-[160px] focus:outline-none focus:ring-0"
                                          :step="1" :placeholder="formatInputRange(needinput, pindex)" />
                                        <div class="text-xs text-red-500 ml-1  min-h-3   w-[250px] max-md:w-[160px]">
                                          {{ inputErrors?.[needinput.inputMeta[pindex].inputId] }}
                                        </div>
                                      </div>
                                    ...
                                    <template v-else-if="getInputConfig(needinput, pindex).viewType === 50">
                                      ...
                                      <div>
                                        ...
                                        <Select v-model:value="needinput.inputvalue[pindex]" allowClear
                                          ...
                                          size="middle" :dropdownStyle="{ lineHeight: '28px' }"
                                          class="custom-select  w-[250px] max-md:w-[160px]" placeholder="Choose a value"
                                          @change="() => changeinputvalue(property, needindex + 2, index)" />
                                        <div class="text-xs text-red-500 ml-1  min-h-4  w-[250px] max-md:w-[160px]">
                                          {{ inputErrors?.[needinput.inputMeta[pindex].inputId] }}
                                        </div>
                                      </div>
                ...
                <p v-if="property.isneedinput" class="text-[14px] text-[#5A5B5B]   cursor-pointer" @click="openChat">
                  Not sure about your measurements?<span class="ml-2 text-primary hover:text-primary-600 underline">Chat
                    with us.</span>
                </p>
              </div>
            </div>
              ...
              <div class="w-full mx-auto bg-white rounded-md text-[#0C1013]">
                ...
                <div class="flex flex-row items-center gap-4 mt-4">
                  ...
                  <div class="flex items-center flex-wrap gap-2">
                    ...
                    <div v-if="offPrice" class="text-[#FF4F63] md:flex hidden text-[14px]">Save ${{ offPrice }} now. Apply
...
const inputErrors = ref({})  // 保存所有输入框的错误信息，供 UI 显示
...
//   if (newVal) {
//     productinfo.value.normalPropertyList.forEach(element => {
//       element.needinputlist.forEach(cItem => {
//         for (let i = 0; i < cItem.inputMeta.length; i++) {
//           if (cItem.inputMeta[i].viewType == 50 && cItem.inputMeta[i].ruleList && cItem.inputMeta[i].ruleList.length > 0) {
//             cItem.inputvalue[i] = cItem.inputMeta[i].ruleList[0]?.start
//           }
//         }
//       })
//     });
...
const openChat = () => {
  const chatElement = document.getElementById('channel-Chatway-huoq7ej1ua')
  if (chatElement) {
...
const toGa4Item = (opts = {}) => {
  ...
  return {
    ...
    price: Number(Number(skuprice.value ?? p.basePrice ?? 0).toFixed(2)),
    currency: 'USD',
    quantity: qty,
    item_variant: opts?.selectSku,
    // 可选
...
const addtocart = async () => {
  ...
  try {
    ...
    // 创建购物车
    const data = { productQuantity: quantity.value, productSkuId: selectsku, opType: 'create' }
    ...
    // GA4
    const gaItem = toGa4Item({ withQuantity: true, selectSku: selectsku })
            ...
            groupsMeta.push({ name: g.groupName, start, length: len })
              ...
              ; (g.inputList || []).forEach(input => {
                inputMeta.push({
                  ...
                  inputStart: input.inputStart,
                  inputEnd: input.inputEnd,
                  inputType: input.inputType, // 默认为数字输入框
                  unit: input.unit || '',
                  viewType: input.viewType !== undefined ? input.viewType : 10, // 默认为输入框
                  ruleList: input.ruleList || []  // 下拉选择框的选项
                })
...
function validateSelectionsInline() {
  ...
  props.forEach(function (prop, idx) {
    if (prop.isneedinput && prop.chooseindex === 2) {
      ...
      var inputValueArr = (picked && Array.isArray(picked.inputvalue)) ? picked.inputvalue : []
      const inputErrorsT = {}
      for (var k = 0; k < picked.inputMeta.length && k < inputValueArr.length; k++) {
        const meta = picked.inputMeta[k]
        const val = inputValueArr[k]
        if (meta.viewType == 10) {
          if ((val === '' || val === null || val === undefined)) {
            Object.assign(inputErrorsT, {
              [meta.inputId]: `Please enter a value`.trim()
            });
            ok = false;
            continue;
          }
          if (meta.inputStart || meta.inputEnd) {
            if (val < meta.inputStart || val > meta.inputEnd) {
              Object.assign(inputErrorsT, {
                [meta.inputId]: `${meta.inputStart} ~ ${meta.inputEnd} ${meta.unit || ''}`.trim()
              });
              ok = false;
              continue;
            }
          }
          if (meta.inputType == 10) { // 整数
            if (!Number.isInteger(Number(val))) {
              Object.assign(inputErrorsT, {
                [meta.inputId]: `Please enter a valid integer`
              });
              ok = false;
            }
          } else if (meta.inputType == 50) { // 1位小数
            if (!/^\d+(\.\d{1})?$/.test(val)) {
              Object.assign(inputErrorsT, {
                [meta.inputId]: `Please enter a number with up to 1 decimal place`
              });
              ok = false;
            }
          } else if (meta.inputType == 100) { // 2位小数
            if (!/^\d+(\.\d{1,2})?$/.test(val)) {
              Object.assign(inputErrorsT, {
                [meta.inputId]: `Please enter a number with up to 2 decimal places`
              });
              ok = false;
            }
          }
        ...
        if (meta.viewType == 50) {
          ...
          if ((val === '' || val === null || val === undefined)) {
            Object.assign(inputErrorsT, {
              [meta.inputId]: `Please select a valid option`
            });
            ok = false;
            continue;
          }
      ...
      inputErrors.value = inputErrorsT  // 保存当前输入框的错误信息，供 UI 显示
    }
  })
  console.log('validateSelectionsInline inputErrors=', props, inputErrors.value)
  return ok
}
...
function getFormattedInputDisplay(property) {
  ...
  if (!inputValues.length || inputValues.some(value => value === null || value === undefined || value === '')) {
    return '';
  }
