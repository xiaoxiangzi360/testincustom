  ...
  <div id="hero-splide" ref="rootEl" class="relative w-full max-row pt-3">
    ...
    <Splide v-if="isReady" ref="carouselRef" :options="computedSplideOptions" class="overflow-hidden relative rounded"
      ...
      <SplideSlide v-for="(s, i) in slides" :key="i">
        <!-- 固定纵横比，避免 CLS -->
        <div class="relative w-full min-h-[300px] md:min-h-0 md:aspect-[21/9] overflow-hidden cursor-pointer"
          @click="s.onClick?.()">
          <picture class="absolute inset-0">
            <!-- 平板：1024–1279 -->
          ...
          <div v-if="s.variant === 'C'" class="h-full flex items-center justify-center font-arial px-4">
            <div class="text-center">
              <div class="flex items-center justify-center">
                <div
                  class="py-4 px-8 max-md:px-2 bg-[#d2727d] bg-opacity-60 flex items-center justify-center text-center flex-col gap-4 rounded-[8px] text-white">
                  <div class="text-[72px] max-md:text-[20px] font-[800] ">{{ s.title }}</div>
                  <div class="text-[28px]  max-md:text-[14px] font-semibold">
                    <div>Up to <span class="text-[48px] font-[800]  max-md:text-[18px]">$60 Off</span> on Orders Over
                      $100</div>
                    <div class="mt-[-8px]">Spend more, save more!</div>
                  </div>
                  <div>
                    <div class="text-[18px] max-md:text-[12px] mb-1" v-for="(item, index) in s.features" :key="index">{{
                      item
                      }}</div>
                  </div>
                </div>
              </div>
              <button class="mt-11 max-md:mt-6 rounded-md px-4 sm:px-5 py-2 sm:py-[14px] bg-[#ed7576] text-white"
                @click="s.onClick?.()">
                {{ s.ctaText || 'View More' }}
              </button>
            </div>
...
/** 原始图片（保持干净，无查询参数） */
const baseImages = [
  'https://cdn.incustom.com/upload/web/home_banner_0130_3.webp',
...
const slides = ref<Slide[]>([
  ...
  // },
  {
    variant: 'C',
    align: 'left',
    title: 'Valentine\'s Day Flash Deal',
    subtitle: 'TAKE UP TO 18% OFF EVERYTHING.',
    features: ['PLUS a Free Charming Necklace Included!'],
    ctaText: 'SHOP NOW',
    onClick: () => { router.push('/activityPage/valentines-day-2002649538686058497') },
  },
  {
    variant: 'A',
    align: 'left',
